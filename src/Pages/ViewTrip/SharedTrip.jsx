...
export default function SharedTrip() {
  ...
  async function fetchSharedTrips() {
    ...
    // 1. Fetch shared trips (Shared Taxi or Shared Bus)
    const { data: tripData, error: tripError } = await supabase
      .from("trips")
      .select("id, userId, routeId, departureTime, arrivalTime, ride_status, status")
      .in("tripType", ["Shared Taxi", "Shared Bus"])   // Update types if they're different in DB
      .order("createdAt", { ascending: false });

    if (tripError) {
      console.error("Error fetching shared trips:", tripError);
      setTrips([]);
      setLoading(false);
      return;
    }
  ...
  return (
    <Box>
      ...
      {loading ? (
        ...
        <Paper sx={{ p: 2 }}>
          <Table size="small" aria-label="shared trips table" stickyHeader>
            <TableHead>
              <TableRow>
